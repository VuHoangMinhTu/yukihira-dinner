...
public class UserSpecification {
    ...
    public static Specification<Users> filterUsers(FilterUserDetailsRequestDTO request) {
        return (root, query, cb) -> {
            List<Predicate> predicates = new ArrayList<>();

            // 1. Join với bảng UserDetails (LEFT JOIN để lấy cả user chưa có detail)
            Join<Users, UserDetails> detailsJoin = root.join("userDetails", JoinType.LEFT);
